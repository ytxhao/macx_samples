...
//    timespec warn_ts = warn_after_ms == kForever ||
//    (give_up_after_ms != kForever && warn_after_ms > give_up_after_ms) ? : GetTimespec(warn_after_ms);
...
//      webrtc::WarnThatTheCurrentThreadIsProbablyDeadlocked();
      std::cout <<  "webrtc::WarnThatTheCurrentThreadIsProbablyDeadlocked()" << std::endl;
      error = wait(give_up_ts);
    }
  }
